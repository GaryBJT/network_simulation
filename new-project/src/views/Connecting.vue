      ...
      <el-main width="auto">
        ...
        @click="handleCanvasClick">
          ...
          @wheel.prevent="handleWheel">
            ...
            <path v-for="(conn, index) in connections" :key="index" :d="calculateConnectionPath(conn)" stroke="black" @contextmenu.prevent="showContextMenu($event, conn)"
              :stroke-width="4* scaleFactor" fill="none"  class="connection"  :class="{ active: selectedConnection === conn.ID }" @click.stop="selectConnection(conn)" />
            <!-- line适用于绘制两点之间的简单直线 -->
            <line v-if="traceMouse.isTracing" :x1="traceMouse.startX" :y1="traceMouse.startY" :x2="traceMouse.cursorX"
              :y2="traceMouse.cursorY" stroke="black"  :stroke-width="4* scaleFactor"/>
            <!-- 绘制设备节点 -->
          ...
          >
            ...
            >
              <el-form-item prop="label" label="名称">
                <el-input  v-model="nodeform.label" id="label"></el-input>
              </el-form-item>
              ...
              <el-form-item prop="DataRate" label="数据传输速率">
                <el-input  v-model="nodeform.DataRate" id="DataRate"></el-input>
              </el-form-item>
        ...
        >
          <el-form-item prop="label" label="名称" >
            <el-input  v-model="linkform.label" id="label"></el-input>
          </el-form-item>
          ...
          <el-form-item prop="delay" label="时延">
            <el-input  v-model="linkform.delay" id="delay"></el-input>
          </el-form-item>
        </el-form>
...
export default {
  data() {
    return {
      ...
      scaleFactor:1,
      dialogVisible:false,//节点界面
      linkVisible:false,//链路界面
      nodeform:{
          label: '',
          application:'',
          potocal:'',
      ...
      selectedConnection: null,// 当前选中的连接线ID
    }
  },
  components: {
    CommonHeader,
  ...
  mounted() {
    ...
    // if (e.key === 'Escape') {this.connectionState = { creating: false, sourceNode: null }}
    // })
    if(this.receivedata){
 ...
 updateSize() {
   ...
   this.height = container.clientHeight;
    ...
    deleteNode() {
      ...
      // console.log(this.contextMenu.targetNode);
      
      if(this.contextMenu.targetNode.type!=='无线链路'&&this.contextMenu.targetNode.type!=='有线链路'){
    ...
    editproperty(){
      ...
      const link=this.connections.find((n)=>n.ID===nodeId)
    ...
    linkSubmit(){
      ...
      for(let i=0;i<this.connections.length;i++){
        if(this.connections[i].ID===test.ID){
          this.connections.splice(i,1,test);
          break
        }
      }
      this.channelClose()
    ...
    handleLinkDragStart(event, device) {
      ...
      // console.log(event.dataTransfer.getData('linkType'));
      this.tempLink=JSON.parse(event.dataTransfer.getData('linkType'))
    ...
    handleCanvasClick(event) {
      ...
      if (event.target === this.$refs.rect) { this.selectedConnection = null }
      ...
      if (nodeElement) {
        ...
        // console.log(nodeId);
        // const link=event.dataTransfer.getData('linkType')
